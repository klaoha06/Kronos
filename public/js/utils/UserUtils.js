define([
  ...
  var UserUtils = {
        ...
        function(response) {
          if (response && !response.error) {
            $.ajax({
              ...
              .done(CalendarActions.addOrUpdateCal)
    ...
    unfollow: function(user_id) {
      $.post(API_URL + '/users/' + user_id + '/unfollow')
        .done(UserActions.unfollowCompleted)
        .fail(UserActions.unfollowFailed);
    },
    follow: function(user_id) {
      $.post(API_URL + '/users/' + user_id + '/follow')
        .done(UserActions.followCompleted)
        .fail(UserActions.followFailed);
    },
    ...
    logOut: function() {
      ...
      // Clearing Server
      $.get(API_URL + '/users/clear_session');
    }
  };
  return UserUtils;
});
